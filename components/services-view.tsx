...
import { whatWeDo, sectors, type Service } from "@/lib/data";
import { FadeUp } from "@/components/animations";
import { ListingCard } from "@/components/listing-card";
import { useLanguage } from "@/components/language-provider";
import type { Dictionary } from "@/lib/i18n/dictionary";

/* One labelled group of cards (a discipline set or a sector set). */
function ServiceGroup({
  ...
  return (
    <div>
      <FadeUp delay={delayBase} className="pb-5 md:pb-6">
        <p style={{ fontFamily: "var(--font-inter)", fontSize: "var(--fs-caption)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.28em", color: "var(--text-dim)" }}>
          {label}
        </p>
      </FadeUp>
      <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 md:gap-12 lg:gap-x-14 lg:gap-y-8">
        {items.map((service, i) => {
          const svc = t.services[service.slug as keyof typeof t.services];
          return (
            <FadeUp key={service.slug} delay={delayBase + 0.08 + i * 0.08}>
              <ListingCard
                href={`/services/${service.slug}`}
                tone={service.tone}
                topLabel={svc?.category ?? service.category}
                image={service.image}
                imageAlt={svc?.title ?? service.title}
              >
                <div className="pt-1">
                  <h2 style={{
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--text)]">
      <main className="pt-[4.2rem]">
        <div className="mx-auto flex w-full max-w-[88rem] flex-col px-5 sm:px-8 md:px-12 lg:px-20">
          ...
          {/* ── SERVICE GROUPS ── */}
          <div className="flex flex-col gap-14 pb-16 pt-8 md:gap-20 md:pb-20 md:pt-10 lg:gap-16 lg:pb-8 lg:pt-8">
            <ServiceGroup label={t.nav.whatWeDo} items={whatWeDo} t={t} delayBase={0.22} />
            <ServiceGroup label={t.nav.whatWeBuild} items={sectors} t={t} delayBase={0.3} />
          </div>
